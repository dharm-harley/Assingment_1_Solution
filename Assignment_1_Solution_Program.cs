...
namespace Assignment1_Spring2021
{
    class Program
    {
        ...
        private static void printTriangle(int n)
        {
            try
            {
                ...
                for (j = 1; j <= n; j++)
                {
                    ...
                    count--;
                    #endregion
                    #region Printing '*' in pyramid form
                    for (i = 1; i <= 2 * j - 1; i++)
                        Console.Write("*");
                    Console.WriteLine();
                    #endregion
        ...
        /// If you add a plus('+') in the local name, everything after the first plus sign will be ignored.This allows certain emails to be filtered, for example ro.cky+bulls @usf.com will be forwarded to rocky@usf.com.  (Again, this rule does not apply for domain names.)
        ...
        private static int UniqueEmails(List<string> emails)
        {
            try
            {
                ...
                foreach (string email in emails)
                {
                    ...
                    string completeEmail = $"{localName}@{domainName}";
                    if (!uniqueEmails.Contains(completeEmail))
                    {
                        uniqueEmails.Add(completeEmail);
                    }
                }
